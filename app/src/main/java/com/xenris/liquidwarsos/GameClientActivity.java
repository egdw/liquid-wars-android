        ...
        if(NativeInterface.teamScore(StaticBits.team) == 0) {
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    builder.setMessage(GameClientActivity.this.getResources().getString(R.string.You_Lose));
                    dialog = builder.show();
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if(p == StaticBits.team)
                            ...
                            builder.setMessage(Util.teamToNameString(p) + GameClientActivity.this.getResources().getString(R.string.wins));
                        dialog = builder.show();
                        TextView messageText = (TextView)dialog.findViewById(android.R.id.message);
                        messageText.setGravity(Gravity.CENTER);
...
//        android.util.Log.i("mylog", xs[1] + " " + ys[1]);
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        if((keyCode == KeyEvent.KEYCODE_BACK) && !gameFinished) {
            ...
            builder.setPositiveButton(R.string.Yes, clicker);
    ...
    public void onServerMessageReceived(int argc, int[] args) {
        if(args[0] == StaticBits.STEP_GAME) {
            ...
            ToastOnUiThread(this.getResources().getString(R.string.Game_canceled));
            running = false;
    ...
    public void onServerConnectionFailed(String ip) {
        ...
        ToastOnUiThread(this.getResources().getString(R.string.Connection_lost));
        running = false;
    }
        ...
        runOnUiThread(new Runnable() {
            ...
            public void run() {
                ...
                if(winningTeam == StaticBits.team)
                    builder.setMessage( GameClientActivity.this.getResources().getString(R.string.Out_of_time_You_win));
                else
                    builder.setMessage( GameClientActivity.this.getResources().getString(R.string.Out_of_time) + Util.teamToNameString(winningTeam) +  GameClientActivity.this.getResources().getString(R.string.wins));
                dialog = builder.show();
                TextView messageText = (TextView)dialog.findViewById(android.R.id.message);
